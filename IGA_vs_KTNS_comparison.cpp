...
unsigned int IGA_bitwise_128(Data& data, std::vector<int>& chromosome) {
    ...
    //assert(KTNS(data, chromosome) == dist);
    return dist;
}

unsigned int IGA_bitwise(Data& data, std::vector<int>& chromosome) {
    ...
    //assert(result==KTNS(data, chromosome));
    return result;
}
...
double algorithm_time(std::string dataset_path, std::string prem_file_path) {
    int n = -1, m = -1, C = -1;
    ...
    std::ifstream fin2(prem_file_path);
    int n_premuts;
    fin2 >> n_premuts;
    std::vector<std::vector<int>> quest(n_premuts, std::vector<int>(n, -1));
...
double dataset_time_catanzaro(std::string folde_name, std::string dataset) {
    ...
    for (int instance_num = 1; instance_num <= 10; instance_num++) {
        ...
        summary_time += algorithm_time(dataset_path, prem_file_path);
    }
    return summary_time;
}
...
double dataset_time_mecler(std::string folde_name, std::string dataset) {
    std::string dir = "./";
    dir += folde_name + "/";
    std::string ABCD = std::string{ dataset[0] } + std::string{ dataset[1] };
...
int main() {
    ...
    func_to_test_name = "IGA_bitwise";
    func_to_test = IGA_bitwise;
    run_tests();
